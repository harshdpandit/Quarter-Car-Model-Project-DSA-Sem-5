...
//
ks=21900;//suspension spring 
cs=900; //suspension damper  
//
...
v=36000/3600; //speed of car gravel road //reducing the speed will help(in report)
...
subplot(2,1,1)
plot(t',y(2,:)',[2])
xlabel('time: seconds');ylabel('states: ms');
...
for i=1:10001,
    if accn(2,i)>(0.2*9.81) then val=accn(2,i)
        disp(val)
    end   
end
...
subplot(2,1,2)
plot(t',accn(2,:)',[2])
xlabel('time: seconds');ylabel('accns: ms');
title("$Evolution\:of\:acceleration\:of\:EVERYONE\:CHEATS\:model$");
legends(["$An_{1}(t)$"],[2],opt="?")



//from here onwards Harsh has done changes to the code
